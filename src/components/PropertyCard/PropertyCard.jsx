    ...
    return (
        <div className='col-lg-4 col-md-6 col-sm-12 mb-4'>
            <div className="card bg-white text-white" style={{ borderRadius: '14px' }}>
                <img src={data.image_urls[0]} height='250' className="card-img" alt="..." style={{ borderRadius: '14px' }} />
                <div className="card-img-overlay">
                    {/* <h5 className="card-title bg-white">Card title</h5> */}
                    <div className='row justify-content-between'>
                        {
                            data.statu ?
                                <span class="property-type bg-accent shadow pl-2 p-1 pr-2" style={{ borderRadius: '5px' }}>{data.statu.name.toUpperCase()}</span> : null
                        }
                        <button className='btn btn-sm bg-white shadow' onClick={() => setState({ ...state, info: !state.info })}>
                            <i className='ti-info'></i>
                        </button>
                    </div>
                    <p className={`card-text text-shadow bg-accent rounded p-1 ${state.info ? 'show' : 'hide'}`}>{data.description}</p>
            ...
            <div className='container-fluid p-0 mt-2'>
                ...
                <div className='d-flex justify-content-between'>
                    ...
                    {
                        data.categorie ?
                            <span><Tag color="green">{data.categorie.name.toUpperCase()}</Tag></span> : null
                    }
                </div>
                <div className='container-fluid'>
                    <div className='row justify-content-start'>
                        <div className='mr-3'>
                            <small >Sitting Room: </small><span><b>{data.sittingroom}</b></span>
                        </div>
                        <div className='mr-3'>
                            <small>Bedroom: </small><span><b>{data.bedroom}</b></span>
                        </div>
                        <div>
                            <small>Toilet: </small><span><b>{data.toilet}</b></span>
                        </div>
                    </div>
                </div>
            </div>
